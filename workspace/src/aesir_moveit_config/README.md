...
This package provides the MoveIt 2 configuration and launch files for the **AESIR robotic arm** (`custom_arm`). It features a complete hybrid control architecture, supporting collision-aware motion planning, real-time servo teleoperation, and low-level joint-by-joint control modes.
...
1. Build the packages:
   ```bash
   colcon build --packages-select aesir_robot_description
   colcon build --packages-select aesir_moveit_config
   ```
...
```bash
ros2 launch aesir_moveit_config demo.launch.py
```
...
```bash
ros2 launch aesir_moveit_config bringup.launch.py
```
...
```bash
ros2 launch aesir_moveit_config bringup_controllers.launch.py
```
...
```bash
ros2 launch aesir_moveit_config commander.launch.py
```
...
```bash
ros2 launch aesir_moveit_config servo_teleop.launch.py
```
...
```bash
ros2 run aesir_moveit_config test_teleop.py
```
...
```bash
ros2 run aesir_moveit_config joint_keyboard.py
```
-  Joint 1: `q/a` : (+/-)
-  Joint 2: `w/s` : (+/-)
-  Joint 3: `e/d` : (+/-)
